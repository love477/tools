...
func download(ch chan string, done chan struct{}, wg *sync.WaitGroup, directory string) {
	for {
		select {
		case uri := <-ch:
			// get filename by uri
...
func DownloadWallhavenToplist(maxPage int, directory string) {
	if maxPage <= 0 {
